...
- [x] (M) Compose: SQL Server container with persistent `luga-sql-data` volume → `luga-core` DB → `api` project (Server.Host). Server.Host already serves the Blazor WASM bundle via `UseBlazorFrameworkFiles` + `MapFallbackToFile`, so WASM and API share the same origin in dev (matching production) — no separate `web` project to avoid the cross-origin/`/api/*`-fallback trap where Refit silently parses `index.html` as JSON. `WaitFor` chained so the API only starts after the DB is healthy
...
- **2026-05-14**: WASM E2E smoke-tested in Chrome against the live Aspire stack — Marketing landing, `/pricing` (3 seeded plans rendered from `/api/marketing/plans`), `/modules`, `/about`, `/contact`, `/login`, all 509 static + WASM assets returned 200, console clean. Two regressions fixed in the process: (1) dropped the dead `<link href="Luga.Client.Host.styles.css">` from `index.html` (no `.razor.css` files exist anywhere, so the bundle was always 404); (2) removed the redundant `web` project from the Aspire AppHost — Server.Host already serves the WASM bundle via `UseBlazorFrameworkFiles` + `MapFallbackToFile`, so running a second WASM-only host on a different port created a cross-origin trap where `/api/*` fell through to `index.html` and Refit silently parsed HTML as JSON ("Nenhum plano publicado" on the pricing page). The single-host layout matches the documented production setup (CLAUDE.md §5.8).
